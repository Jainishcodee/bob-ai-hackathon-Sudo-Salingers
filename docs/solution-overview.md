...
Pharos deliberately stops at the numbers. The MCP server ([`src/mcp_server/server.py`](../src/mcp_server/server.py)) exposes six tools and two prompt templates; **Bob is the analyst**:
...
4. **Date it** → `signal_emergence_timeline` establishes when the lead signal first met the criteria, whether another drug's reporting masked it, and the lead time versus regulatory action.
5. **Write** → Bob produces a BLUF-style memo: verdict, evidence table, emergence year, biases, next step.
